...
make_config_for_test(const mocks::Network* env, std::string_view ini_str = "")
{
  ...
  conf->beldexd.whitelistRouters = false;
...
const std::string ini_minimal = "[beldexd]\nrpc=ipc://dummy\n";

TEST_CASE("master node bind section on valid network", "[config]")
{
...
TEST_CASE("master node bind section on nat network", "[config]")
{
  std::unordered_multimap<std::string, llarp::IPRange> env{
...
TEST_CASE("master node bind section with multiple public ip", "[config]")
{
  std::unordered_multimap<std::string, llarp::IPRange> env{
      {"mock0", llarp::IPRange::FromIPv4(1, 1, 1, 1, 32)},
