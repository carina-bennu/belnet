...
local docker_base = 'registry.beldex.rocks/'; 
...
local ci_dep_mirror(want_mirror) = (if want_mirror then ' -DLOCAL_MIRROR=https://beldex.rocks/deps ' else '');
    ...
    submodules,
    {
      ...
      commands: [
                  ...
                  if beldex_repo then [
                    ...
                    'echo deb http://deb.beldex.io $$(lsb_release -sc) main >/etc/apt/sources.list.d/beldex.list',
  ...
  steps: [
    submodules,
    {
      ...
      commands: [
        ...
        'git clone https://github.com/beldex-io/belnet-flutter-app belnet-mobile',
        'cp -av build-android/out/* belnet-mobile/belnet_lib/android/src/main/jniLibs/',
        'cd belnet-mobile',
        'flutter build apk --debug',
        'cd  ..',
        'cp belnet-mobile/build/app/outputs/apk/debug/app-debug.apk belnet.apk',
      ] + extra_cmds,
    },
  ],
};
  ...
  steps: [
    ...
    },
    {
      ...
      commands: [
        ...
        'JOBS=' + jobs + ' VERBOSE=1 ./contrib/windows.sh -DSTRIP_SYMBOLS=ON -DGUI_EXE=$${DRONE_WORKSPACE}/gui/release/Belnet-GUI_portable.exe' +
        ci_dep_mirror(local_mirror),
      ] + extra_cmds,
    },
  ],
    ...
    submodules,
    {
      ...
      commands: [
        ...
        'echo "man-db man-db/auto-update boolean false" | debconf-set-selections',
             ...
             'cp contrib/deb.beldex.io.gpg /etc/apt/trusted.gpg.d',
             'echo deb http://deb.beldex.io $${distro} main >/etc/apt/sources.list.d/beldex.list',
           ] else []) + [
        apt_get_quiet + ' update',
        apt_get_quiet + ' install -y eatmydata',
        ...
        |||
          ...
          if ! git checkout $${distro_branch}; then
              git remote add --fetch upstream https://github.com/beldex-io/belnet.git &&
...
[
  ...
  // Static build (on focal) which gets uploaded to builds.belnet.dev:
   debian_pipeline('Static (focal amd64)',
                  ...
                  cmake_extra='-DBUILD_STATIC_DEPS=ON -DBUILD_SHARED_LIBS=OFF -DSTATIC_LINK=ON ' +
                              ...
                              '-DNATIVE_BUILD=OFF -DWITH_SYSTEMD=OFF -DWITH_BOOTSTRAP=OFF -DBUILD_LIBBELNET=OFF',
                  extra_cmds=[
