...
namespace llarp
{
  ...
  bool
  KeyManager::initialize(const llarp::Config& config, bool genIfAbsent, bool isMNode)
  {
    if (m_initialized)
      return false;
    ...
    // valid signature
    m_needBackup = (isMNode and not rc.VerifySignature());
    ...
    if (not config.beldexd.whitelistRouters)
    {
      // load identity key or create if needed
      auto identityKeygen = [](llarp::SecretKey& key) {
        // TODO: handle generating from master node seed
