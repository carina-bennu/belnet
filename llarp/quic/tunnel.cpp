...
namespace llarp::quic
{
  namespace
  {
    ...
    on_outgoing_data(uvw::DataEvent& event, uvw::TCPHandle& client)
    {
      ...
      LogTrace(peer.ip, ":", peer.port, " → belnet ", buffer_printer{data});
  ...
  TunnelManager::make_server()
  {
    ...
    server_->stream_open_callback = [this](Stream& stream, uint16_t port) -> bool {
      ...
      auto belnet_addr = var::visit([](auto&& remote) { return remote.ToString(); }, *remote);
      auto tunnel_to = allow_connection(belnet_addr, port);
      if (not tunnel_to)
        return false;
      LogInfo("quic stream from ", belnet_addr, " to ", port, " tunnelling to ", *tunnel_to);
  ...
  TunnelManager::allow_connection(std::string_view belnet_addr, uint16_t port)
  {
    for (auto& [id, handler] : incoming_handlers_)
    {
      try
      {
        if (auto addr = handler(belnet_addr, port))
          return addr;
      }
    ...
    LogWarn(
        "Incoming quic connection from ", belnet_addr, " to ", port, " declined by all handlers");
    return std::nullopt;
  }
