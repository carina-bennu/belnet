...
#if 0
#define BELNET_LOG_DEPRECATED(Meth) \
...
namespace llarp
{
  ...
  template <typename... T>
  struct BELNET_LOG_DEPRECATED(Trace) LogTrace : log::trace<T...>
  {
    LogTrace(
        T&&... args,
  ...
  template <typename... T>
  struct BELNET_LOG_DEPRECATED(Debug) LogDebug : log::debug<T...>
  {
    LogDebug(
        T&&... args,
        const log::slns::source_location& location = log::slns::source_location::current())
  ...
  template <typename... T>
  struct BELNET_LOG_DEPRECATED(Info) LogInfo : log::info<T...>
  {
    LogInfo(
        T&&... args,
        const log::slns::source_location& location = log::slns::source_location::current())
  ...
  template <typename... T>
  struct BELNET_LOG_DEPRECATED(Warning) LogWarn : log::warning<T...>
  {
    LogWarn(
        T&&... args,
        const log::slns::source_location& location = log::slns::source_location::current())
  ...
  template <typename... T>
  struct BELNET_LOG_DEPRECATED(Error) LogError : log::error<T...>
  {
    LogError(
        T&&... args,
        const log::slns::source_location& location = log::slns::source_location::current())
