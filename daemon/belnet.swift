...
// - we have three "bundles" we need to manage: the GUI app, the system extension, and the Belnet
...
//   extensions.
// - if we embed Belnet.app inside Belnet-GUI.app and then the system extension inside Belnet.app
...
//   script to execs the embedded GUI app) inside the Belnet.app and make the GUI the default for
//   the application then Belnet gets killed by gatekeeper because code signing only applies the
...
// exec'ing the binary and loading belnet, depending on how it is called.
//
...
//   - `--start` -- load the system extension and start belnet
//   - `--stop` -- stop belnet
//
// macOS: land of half-baked implementations and nasty hacks to make anything work.
if args.count == 1 {
    let gui_path = Bundle.main.resourcePath! + "/../Helpers/Belnet-GUI.app"
